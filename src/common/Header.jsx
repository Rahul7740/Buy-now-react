          ...
          <div className="header-section">
            ...
            <div className="searchBar-and-user-container">
              ...
              <div className="user-container">
                ...
                <button onMouseOver={() => { setLanguagPopup(true) }} onMouseLeave={() => { setLanguagPopup(false) }} className="languages-btn">
                  <img onClick={showLanguagePopup} className="language" src={require(`../assets/svg/languages/${languag}`)} alt="langauge" />
                  <img onClick={showLanguagePopup} style={{ transition: "all 0.3s", transform: langaugPopup === true ? "rotate(180deg)" : "", mixBlendMode: "color-burn" }} src={require(`../assets/svg/language-menu-btn.svg`).default} alt="languagess" />
                  <div style={{ transition: "all 0.2s", opacity: langaugPopup === true ? "1" : "0", zIndex: langaugPopup === true ? "999999" : "-444" }} className="language-popup">
                    <div className="language-popup-searchBar ">
                      <img src={SvgPath.searchIcon} alt="search" />
                      <input type="search" onChange={(e) => { setLanguagSearch(e.target.value) }} value={languagSearch} placeholder="Search" />
                    </div>
                    <div className="language-popup-btns">
                      {
                        language.map((item, index) => (
                          item.name.toLowerCase().includes(languagSearch.toLowerCase()) ?
                            <div onClick={() => { setLanguag(item.image); setLanguagPopup(false); setLanguagSearch("") }} key={index}>

                              <h3>{item.name} -<span style={{ color: "#ABB5BA" }}> {item.shortcut}</span></h3>
                              <img className="right-arrrow" src={require(`../assets/svg/right-victor-perpul.svg`).default} alt="go" />
                            </div> : ""
                        ))
                      }
                    </div>
                ...
                <button className="header-user-btn">
                  <svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <rect className="user-icon-border" x="0.5" y="0.5" width="43" height="43" rx="21.5"  stroke-opacity="0.08" />
                    <path className="user-icon" d="M22 15.9C23.16 15.9 24.1 16.84 24.1 18C24.1 19.16 23.16 20.1 22 20.1C20.84 20.1 19.9 19.16 19.9 18C19.9 16.84 20.84 15.9 22 15.9ZM22 24.9C24.97 24.9 28.1 26.36 28.1 27V27.1C28.1 27.6523 27.6523 28.1 27.1 28.1H16.9C16.3477 28.1 15.9 27.6523 15.9 27.1V27C15.9 26.36 19.03 24.9 22 24.9ZM22 14C19.79 14 18 15.79 18 18C18 20.21 19.79 22 22 22C24.21 22 26 20.21 26 18C26 15.79 24.21 14 22 14ZM22 23C19.33 23 14 24.34 14 27V29C14 29.5523 14.4477 30 15 30H29C29.5523 30 30 29.5523 30 29V27C30 24.34 24.67 23 22 23Z"  />
                  </svg>
                  <div className="accout-popup">
                    <div className="welecome-text-accoutPopup">
                      <h3>WelCome</h3>
                      <img src={require(`../assets/svg/main-logo-icon.svg`).default} alt="logo" />
                    </div>
                    ...
                    <div className="accoutPopup-login-btns">
                      <Link style={{ background: "#422659", color: "#FFFFFF" }}>LOG IN</Link>
                      <Link>SIGN UP</Link>
                    </div>
                  </div>
                </button>
